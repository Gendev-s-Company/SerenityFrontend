"use client";
import { getAllPlatALL } from "@/infrastructure/restaurant/plat/plat/platRequest";
...
export default function Catalogue() {
  const [Plat, setPlat] = useState<PlatCatalogueEntity[]>([]);
  const user = getLocalStorage()!;

  useEffect(() => {
    if (user && user.profil.company.companyID) {
      getAllPlatALL(user.profil.company.companyID)
  ...
  const totalPages = Math.ceil(Plat.length / ITEMS_PER_PAGE);

  // Génère exactement 12 slots pour garder la grille 3x4 stable
  const pageSlots = useMemo(() => {
    const items = Plat.slice(currentPage * ITEMS_PER_PAGE, (currentPage + 1) * ITEMS_PER_PAGE);
  ...
  const selectedItems = useMemo(() => {
    return Plat.filter(item => cart[String(item.dishID)] > 0).map(item => ({
      ...item,
      quantity: cart[String(item.dishID)] || 0
    }));
  }, [cart, Plat]);

  const totalPrice = selectedItems.reduce((acc, item) => acc + (item.price.price * item.quantity), 0);
  ...
  const convertToBase64 = (buffer: number[], type: string) => {
    ...
    //   return Plat.map(plat => ({
    //     ...plat,
    //     photos: plat.photos.map(photo => ({
    //       ...photo,
    //       src: photo?.files?.data
    //         ? convertToBase64(photo.files.data, photo.files.type)
    //         : photo?.path || "/placeholder-food.jpg"
    //     }))
    //   }));
    // }, [Plat]);

    // console.log("TEST", displayPhotos.map(p => p.photos.map(ph => ph.src)));
  ...
  return (
    <main className="min-h-screen bg-[#f3f0eb] flex flex-col items-center justify-center p-4 font-serif">
      ...
      <div className="relative flex items-center max-w-6xl w-full gap-4">
        ...
        <button onClick={() => setCurrentPage(p => Math.max(0, p - 1))} disabled={currentPage === 0} className="p-3 disabled:opacity-0 text-[#4a3427]">
          <ChevronLeft size={44} />
        </button>

        <div className="bg-[#4a3427] p-2 rounded-2xl shadow-2xl flex-1">
          <div className="bg-white rounded-xl p-10 min-h-[850px] flex flex-col border border-gray-100">
            ...
            <div className="flex-1 grid grid-cols-3 grid-rows-4 gap-x-8 gap-y-4">
              {pageSlots.map((item, index) => {
                const slotKey = item.isPlaceholder ? `empty-${index}` : `dish-${item.dishID}`;
                
                return (
                  <div key={slotKey} className="flex flex-col items-center justify-start min-h-[220px]">
                  {!item.isPlaceholder ? (
                    <div className="flex flex-col items-center text-center w-full animate-in fade-in duration-500 py-4">
                      
                      <a href={`/view/restaurant/plat/detail?dishID=${item.dishID}&dishState=${item.state}`}>
              ...
              {[...Array(totalPages)].map((_, i) => (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all duration-300 ${
                    currentPage === i ? 'w-10 bg-[#4a3427]' : 'w-2 bg-gray-200 hover:bg-gray-300'
                  }`}
                />
              ))}
            </footer>
        ...
        <button onClick={() => setCurrentPage(p => Math.min(totalPages - 1, p + 1))} disabled={currentPage === totalPages - 1} className="p-3 disabled:opacity-0 text-[#4a3427]">
              ...
              {selectedItems.length > 0 ? (
                <>
                  {selectedItems.map((item) => (
                    <div key={item.dishID} className="flex items-center justify-between border-b border-gray-50 pb-4">
                      <div className="flex items-center gap-4 text-gray-800">
                        <div className="relative w-14 h-14 rounded-full overflow-hidden border">
                          <Image src={item.photos[0].files.data? convertToBase64(item.photos[0].files.data, item.photos[0].files.type): item.photos[0]?.path|| "/placeholder-food.jpg"} alt={item.name} fill className="object-cover" />
                        </div>
                        <div>
                      ...
                      <div className="flex items-center gap-3 bg-gray-50 px-3 py-1.5 rounded-xl border">
                        <button onClick={() => updateQuantity(item.dishID, -1)}><Minus size={14}/></button>
                        <span className="font-bold text-sm">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.dishID, 1)}><Plus size={14}/></button>
                      </div>
                    </div>
                  ))}
                  <div className="pt-4 flex justify-between items-center border-t border-[#4a3427]/10">
                    <span className="uppercase tracking-widest text-xs text-gray-400">Total à payer</span>
                    <span className="text-2xl font-bold text-[#4a3427]">{totalPrice} Ar</span>
                  </div>
                </>
              ) : (
                <div className="text-center py-10 italic text-gray-400">Votre panier est vide</div>
              )}
            </div>

            <div className="p-6 bg-gray-50 border-t flex gap-4">
              <button onClick={() => setIsModalOpen(false)} className="flex-1 py-4 border-2 border-[#4a3427] text-[#4a3427] rounded-xl font-bold uppercase text-[10px]">
                Retour
              </button>
              <button disabled={selectedItems.length === 0} className="flex-1 py-4 bg-[#4a3427] text-white rounded-xl font-bold uppercase text-[10px] disabled:opacity-50">
                Confirmer
              </button>
            </div>
          </div>
