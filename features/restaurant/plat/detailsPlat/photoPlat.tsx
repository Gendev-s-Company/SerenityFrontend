...
export default function PhotoDetailPlat({ platID }: PhotoDetailPlatProps) {
  const [photos, setPhotos] = useState<PlatPhotoEntity[]>([]);
  ...
  const fetchPhotos = async () => {
    setLoading(true);
    try {
      const response = await getAllphoto(platID, page, size);
  ...
  const handleUpload = async () => {
    ...
    formData.append("activityID", platID);
