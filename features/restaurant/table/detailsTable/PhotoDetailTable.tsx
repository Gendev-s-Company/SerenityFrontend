...
import { createPhoto, deleteRoomPhoto, getAllphoto } from "@/infrastructure/restaurant/table/tabledetail/tableDetailRequest"

interface PhotoDetailTableProps {
  tableId: string;
}

export default function PhotoDetailTable({ tableId }: PhotoDetailTableProps) {
  const [photos, setPhotos] = useState<TablePhotoEntity[]>([]);
  ...
  const fetchPhotos = async () => {
    setLoading(true);
    try {
      const response = await getAllphoto(tableId, page, size);
  ...
  const handleUpload = async () => {
    ...
    formData.append("activityID", tableId);
